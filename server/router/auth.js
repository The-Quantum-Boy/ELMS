...
router.post('/registerEmp', async (req, res) => {
   const { empId, gender, dob, firstName, lastName, departmentName, address, email, city, country, phone, password, cpassword } = req.body;
   if (!empId || !gender || !dob || !firstName || !lastName || !departmentName || !address || !email || !city || !country || !phone || !password || !cpassword) {
   ...
   try {
      ...
      if (empExist) {
         ...
         const employee = new Employee({ empId, gender, dob, firstName, lastName, departmentName, address, email, city, country, phone, password, cpassword, empStatus: 1 });//if both key and value are same no need to write both for ex. name:name= name
   ...
   try {
      ...
      if (employeeLogin) {
         ...
         if (!isMatch) {
           ...
           res.status(403).json({ message: "employee is not registered please contact admin"});
...
router.post('/addLeave', leaveauth, async (req, res) => {
   ...
   try {
      const leave = new Leaves({ leaveType, fromDate, toDate, description, status: 2, empId: req.empId, firstName: req.firstName, lastName: req.lastName, adminRemark:2  });
      await leave.save();
      console.log("Leave applyeed successfully!");
      res.status(201).json({ message: "Leave applyeed successfully!" });
   } catch (err) {
      console.log(err);
   }
...
router.post('/addAction', async (req, res) => {
   ...
   // update one value 
   try {
      Leaves.findOneAndUpdate({ "_id": objId }, { "$set": { adminRemark: admnAction,status: status,adminRemarkDesc: admnDesc,adminRemarkDate:Date.now() }}).exec(function(err, Leaves){
         if(err) {
             console.log(err);
...
router.post('/UpdateEmp', async (req, res) => {
   const { empId, gender, dob, firstName, lastName, departmentName, address,city,email, country, phone, password, cpassword } = req.body;
   console.log(req.body);
   if (!empId || !gender || !dob || !firstName || !lastName || !departmentName || !email || !address || !city || !country || !phone) {
      return res.status(422).json({ error: "please fill the field properly" });
   }
   try {
     Employee.findOneAndUpdate({ "empId": empId }, { "$set": {  empId, gender, dob, firstName, lastName, departmentName,email, address,city, country, phone, password, cpassword  }}).exec(function(err, Employee) {
         if(err) {
             console.log(err);
             res.status(500).send(err);
         } else {
...
router.post('/changestatus', async (req, res) => {
   ...
   try {
        if(empStatus === 0){
         Employee.findOneAndUpdate({"empId": empId }, { "$set": {empStatus: 1}}).exec(function(err, Employee) {
            if(err) {
                console.log(err);
                res.status(500).send(err);
            } else {
         ...
         Employee.findOneAndUpdate({"empId": empId }, { "$set": {empStatus: 0}}).exec(function(err, Employee){
            if(err) {
                console.log(err);
                res.status(500).send(err);
            } else {
